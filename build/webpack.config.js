...
const config = {
  entry: {
    normalize: ["babel-polyfill", "whatwg-fetch"],
    main: [inProjectSrc(project.jsDir + "/" + project.main)]
  },
  devtool: __DEV__ ? (project.sourcemaps ? "eval-source-map" : false) : false,
  output: {
    path: inProject(project.outDir),
    filename: __DEV__
      ? project.jsDir + "/[name].js"
      : project.jsDir + "/[name].[chunkhash].js",
      ...
      options: {
        ...
        presets: [
          ...
          [
            ...
            {
              modules: false,
              targets: Object.assign({}, project.targetEnvironment, {
                uglify: true
              })
            }
          ]
  ...
  loader: extractStyles.extract({
    ...
    use: [
      ...
      },
      {
        ...
        options: {
          sourceMap: __DEV__ ? project.sourcemaps : false,
          includePaths: [inProjectSrc(project.stylesDir)]
        }
      },
      {
...
if (__STG__ || __PROD__) {
  ...
  const pathsToClean = [project.outDir];
  const cleanOptions = {
  ...
  config.plugins.push(
    new CopyWebpackPlugin([
      {
        from: inProject(project.srcDir + "/" + project.i18nDir),
        to: inProject(project.outDir + "/" + project.i18nDir)
      },
      {
        from: inProject(project.srcDir + "/" + project.assetsDir + "/datas"),
        to: inProject(project.outDir + "/" + project.assetsDir + "/datas")
      },
      {
        from: inProject(project.srcDir + "/" + project.assetsDir + "/fonts"),
        to: inProject(project.outDir + "/" + project.assetsDir + "/fonts")
      },
      {
        from: inProject(project.srcDir + "/" + project.assetsDir + "/vendors"),
        to: inProject(project.outDir + "/" + project.assetsDir + "/vendors")
      },
      {
        from: inProject(project.srcDir + "/favicon.ico"),
        to: inProject(project.outDir + "/")
      },
      {
        from: inProject(project.srcDir + "/manifest.json"),
        to: inProject(project.outDir + "/")
      }
    ])
  );
